...
WINDOW_START = np.datetime64("2026-06-15T07:06:50")
WINDOW_END   = np.datetime64("2026-06-15T07:12:20")

OLA_FOLDER  = HERE / "BOOT_000009"
SFY_NETCDF  = HERE / "test_spikes.nc"
...
# Outlier-filter relaxation. The AHRS outlier filter (global magnitude-MAD +
# local Hampel spike rejection) is tuned for ordinary wave data, where it only
# trims end-of-stream garbage. The violent-lift timing test produces genuine
# accelerations up to ~20 m/s², which look exactly like outliers — with the
# defaults they get clipped (peak |accel| 20.9 -> 14.8 m/s², ~3% of samples
# held), flattening the very impulses we want as timing fiducials. Set True for
# the lift test to disable the amplitude-based spike filters (the decoder's own
# bad-sample flags are still honoured). Leave False for normal wave comparison.
RELAX_OUTLIER_FILTER = True
...
def main() -> None:
    ...
    # Optionally disable the amplitude-based outlier filters so the violent-lift
    # impulses are not clipped (see RELAX_OUTLIER_FILTER). The decoder's own
    # bad-sample flags (use_outlier_flags) are left on.
    if RELAX_OUTLIER_FILTER:
        _method_kwargs = dict(_method_kwargs,
                              magnitude_mad_threshold=float("inf"),
                              hampel_window_seconds=0.0)
    res = _ahrs_fn(
    ...
    # imu_micros elapsed). The Artemis crystal here is ~+450 ppm fast, so we
    # must apply the SAME GNSS rate correction (slope) used for the per-sample
    # UTC mapping before re-anchoring — otherwise world_z runs ~0.15 s fast vs
    # the GNSS-true acc_x_raw across this window (a growing phase shift, worst
    # at the end). slope is UTC-seconds per MCU-microsecond, so slope/1e-6
    # converts MCU-elapsed-seconds -> UTC-elapsed-seconds.
    # (Assumes OLA_IMU_RATE_CORRECTION == 1.0, i.e. PLL-fixed firmware; res.t is
    # built from the uncorrected imu_micros.)
