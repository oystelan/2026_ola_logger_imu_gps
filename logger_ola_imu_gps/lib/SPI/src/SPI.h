...
public:
  ...
  // Project-local extension: do an IOM transaction with an instruction phase
  // (address byte) followed by a half-duplex data phase. Replaces the
  // `transfer(addr); for (...) transfer(0);` pattern, which on Apollo3 v1.2.3
  // leaves leftover bytes in the IOM FIFO and corrupts subsequent transfers.
  //
  // direction: AM_HAL_IOM_RX (read) or AM_HAL_IOM_TX (write).
  // For RX: data_len must be >= 4 (the HAL FIFO loop won't read shorter tails).
  //         The caller is responsible for padding & discarding extras.
  // Returns the HAL status (0 = AM_HAL_STATUS_SUCCESS).
  uint32_t transferInstruction(uint32_t instr,
                               uint8_t  instr_len,
                               void*    tx_buf,
                               void*    rx_buf,
                               size_t   data_len,
                               am_hal_iom_dir_e direction);
