...
uint32_t SPIClass::transferInstruction(uint32_t instr,
                                       uint8_t  instr_len,
                                       void*    tx_buf,
                                       void*    rx_buf,
                                       size_t   data_len,
                                       am_hal_iom_dir_e direction)
{
  ...
  t.uPeerInfo.ui32SpiChipSelect = 0;
  t.ui32InstrLen   = instr_len;
  t.ui32Instr      = instr;
  t.ui32NumBytes   = data_len;
  t.eDirection     = direction;
  t.pui32TxBuffer  = static_cast<uint32_t*>(tx_buf);
  t.pui32RxBuffer  = static_cast<uint32_t*>(rx_buf);
  t.bContinue      = false;
  t.ui8RepeatCount = 0;
  t.ui8Priority    = 1;
  t.ui32PauseCondition = 0;
  t.ui32StatusSetClr   = 0;

  if (direction == AM_HAL_IOM_FULLDUPLEX) {
    return am_hal_iom_spi_blocking_fullduplex(_handle, &t);
  }
  return am_hal_iom_blocking_transfer(_handle, &t);
}
